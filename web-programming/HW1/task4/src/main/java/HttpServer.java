...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file;
        String uri = java.net.URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8);
        if (uri.isEmpty() || uri.equals("/")) {
            file = new File(root, "index.html");
        } else if (uri.equals("/images") || uri.equals("/images/")) {
            file = new File(root, "images/index.html");
        } else {
            file = new File(root, uri);
        }
        long ETag = file.lastModified() + file.length();
        if (request.getHeader("If-None-Match") != null && request.getHeader("If-None-Match").equals(Long.toString(ETag))) {
            response.setHeader("ETag", Long.toString(ETag));
