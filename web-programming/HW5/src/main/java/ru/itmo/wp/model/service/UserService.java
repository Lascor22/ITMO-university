...
public class UserService {
    ...
    public void validateRegistration(User user, String password, String checkPassword) throws ValidationException {
        ...
        if (user.getEmail().length() < 4) {
            throw new ValidationException("Email can't be shorter than 4 characters");
        }
    ...
    public User validateAndFindByLoginOrEmailAndPassword(String loginOrEmail, String password) throws ValidationException {
        User user;
        if (isCorrectEmail(loginOrEmail)) {
    ...
    private boolean isCorrectEmail(String email) {
        return email.contains("@") && !email.replace("@", "").contains("@");
    }
}
