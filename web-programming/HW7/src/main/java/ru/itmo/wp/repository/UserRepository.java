...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // UPDATE `user` SET `disable` = '1' WHERE `user`.`id` = 14

    @Transactional
    @Modifying
    @Query(value = "UPDATE `user` SET `disable` = ?2 WHERE `user`.`id` = ?1", nativeQuery = true)
    void updateDisabled(long userId, boolean change);
